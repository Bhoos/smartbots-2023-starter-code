...
pub fn make_move(payload: &payload_types::PlayPayload) -> Action {
    if payload.played.len() == 0 {
        return Action::Play(PlayAction::CardThrow(Card::new(
            payload.cards.last().unwrap(),
        )));
    }
    ...
    if let Some(card_choices) = same_suit_cards {
        // I have same suit card, must throw that
    ...
    match (&payload.trump_suit, &payload.trump_revealed) {
        (TrumpSuit(Some(suit)), trump_revelation) => {
            // Trump suit I know, (revealed or me-bidder)
            let trump_suit_cards =
                cards::get_same_suit_cards_if_available(&my_cards, Suit::from_string(&suit));

            if let TrumpRevealed(Some(trump_revealer)) = trump_revelation {
                // Trump revealed in game
                ...
                if trump_revealed_this_round && i_revealed_trump {
                    // I am not a bidder
                    if let Some(trump_suit_cards) = trump_suit_cards {
                        // i throw best card
                        return Action::Play(PlayAction::CardThrow(
                            *trump_suit_cards.last().unwrap(),
                        ));
                    } else {
                        // i throw any card
                ...
                // I know the trump card, and it is not revealed
                if let Some(trump_suit_cards) = trump_suit_cards {
                    // i reveal and thow best card
                    return Action::Play(PlayAction::Trump(TrumpCase::RevealAndThrow(
                        *trump_suit_cards.last().unwrap(),
                    )));
                } else {
                    // i throw any card
        ...
        (TrumpSuit(None), _) => {
            // I don't know the unrevealed trump suit, maybe I can just win by revealing it
